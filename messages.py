...
admin = {
    ...
    'markup': [
        ['Сделать рассылку'], ['Модерация', 'изменить баланс', 'пополнить баланс', 'заявки на вывод'], ['⛔️ Отмена']
    ]
}
...
1.Запрещено⛔️:
 1.1. Отписываться от канала в течение 7 дней. 
 1.2. Спамить бота повторными командами. 
 1.3. На один кошелек выводить средства с 2 и более аккаунтов.
 

 За несоблюдение правил, баланс пользователя обнуляется. 


2.Рекламодателям запрещено⛔️:
 2.1. Рекламировать посты и каналы порнографического и мошеннического содержания (всякие схемы, якобы "обыгрывания казино" и т.д.) 
 2.2. Заниматься негативным психологическим воздействием, пропагандой наркотиков и терроризма.
 При обнаружении, мы будем вынуждены заблокировать пользователя полностью, без возврата средств.
 2.3. После оформления заказа убирать права у бота.
 В случае обнаружения ваш заказ удаляется, деньги не возвращаются.
 Права у бота можно убрать после окончания заказа!
 2.4. Запрещено выводить средства полученные на рекламу, посредством бонуса
 
 3. Любой из пунктов может быть изменен, удален или добавлен администрацией в любой момент, без предупреждения пользователей. 
 
 4. Администратор бота не несёт ответственности за посты и каналы раскручиваемые рекламодателями, а также за рекламную рассылку 
 
 5. Администратор бота всегда прав.
 
 6. Пользуясь данным ботом, вы автоматически соглашаетесь с данными правилами. 
...
➖➖➖➖➖➖➖➖➖➖➖➖
1️⃣
За каждого приведенного друга в нашего бота вы получаете 0.5р.
...
rules3 = {
    'text': '''Хочешь такого бота? Пиши @iron_men''',
    'markup': start['markup']
}
...
Выберите услугу в меню👇🏻""",
    'markup': [['👀 Просмотры', '👥 Подписки'], ['📢Заказать рассылку📢'], ['🔖 Мои заказы'], ['🔙 Начало']],
}
for_advert_admin = {
