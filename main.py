...
💸 Выведено всего: *{}*p
    ...
    if text == 'Другой способ':
        return send_message(message, {'text': 'Чтобы пополнить баланс другим способом, пишите @seryoga205', 'markup': messages.start['markup']}, parse_mode="Markdown")
    if text == '🔖 Мои заказы':
...
🔧🔨Чтобы добавить ваш канал для раскрутки - действуйте по инструкции, указанной выше.''', 'markup': messages.for_subs['markup']})
    ...
    if text == '📢Заказать рассылку📢':
        return send_message(message, {'text': '''По поводу Рекламной рассылки обращаться в лс @seryoga205''', 'markup': messages.start['markup']}, parse_mode="Markdown")
...
def ymon():
    token = api.Wallet.get_access_token(client_id='69188F9089187F612B7AD18B26A840444D867FD5F816B94F8CF14B49ABABFC4B', code=flask.request.args['code'],
